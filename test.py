...
def test_linear_slice():
    material_id = "mp-775989"
    chgcar = Chgcar.from_file(CHGCAR_DIRECTORY/f"{material_id}.chgcar")
    sites = chgcar.structure.sites

    expected = chgcar.linear_slice(sites[0].frac_coords, sites[7].frac_coords)
    actual = linear_slice(sites[0].frac_coords, sites[7].frac_coords, (0, 0, 0), chgcar)
